...
export default function ExternalTransfer() {
  ...
  const [receiverBankBicfi, setReceiverBankBicfi] = useState('BANK_B');
  const [amount, setAmount] = useState('');
  const [title, setTitle] = useState('');
  const [routingSystem, setRoutingSystem] = useState<RoutingSystem>('ELIXIR');
  ...
  const isSorbnet = routingSystem === 'SORBNET';
  ...
  const loadAccounts = async () => {
    try {
      ...
      setError("Nie udało się pobrać Twoich kont.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!receiverName.trim()) errors.receiverName = "Podaj nazwę odbiorcy.";
    if (!receiverBankBicfi.trim()) errors.receiverBankBicfi = "Podaj BICFI banku odbiorcy.";

    const amt = parseFloat(amount);
    if (!amount || amt <= 0) errors.amount = "Podaj kwotę większą od 0.";
    if (isSorbnet && amt < 1000) {
      errors.amount = "SORBNET przyjmuje przelewy od 1000 PLN.";
    }
    if (!title.trim()) errors.title = "Tytuł nie może być pusty.";
  ...
  const confirmWithPin = async () => {
    ...
    try {
      ...
      if (status === 'REJECTED') {
        setError(`Przelew odrzucony: ${res.data?.rejectionReason || ''}`);
      } else {
        const msg =
          status === 'PROCESSED' ? 'Przelew zrealizowany.' :
          status === 'SENT' ? 'Przelew wysłany do rozliczenia (oczekuje na sesję).' :
          'Przelew przyjęty do realizacji.';
      ...
      <nav className="bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-zinc-500 text-sm hidden md:inline">/ Przelew zewnętrzny</span>
          </div>
          <div className="flex items-center gap-6">
            <div className="hidden md:flex flex-col text-right">
              <span className="text-zinc-200 text-sm font-medium">{user?.email}</span>
              <span className="text-zinc-500 text-xs">Numer klienta: {user?.customerNumber}</span>
            </div>
      ...
      <main className="flex-grow p-4 md:p-8 flex justify-center items-start pt-12">
        <div className="w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8 shadow-xl">
          <div className="mb-8 text-center">
            <h2 className="text-2xl font-bold">Przelew zewnętrzny</h2>
            <p className="text-zinc-500 text-sm mt-1">ELIXIR / Express Elixir / SORBNET</p>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">System rozliczeniowy</label>
              <div className="grid grid-cols-3 gap-2">
                {(['ELIXIR', 'EXPRESS', 'SORBNET'] as RoutingSystem[]).map(sys => (
                  <button key={sys} type="button" onClick={() => setRoutingSystem(sys)}
              ...
              <p className="text-xs text-zinc-500 mt-1.5">
                {routingSystem === 'ELIXIR' && 'Sesyjny, taniej, kilka godzin.'}
                {routingSystem === 'EXPRESS' && 'Natychmiastowy, 24/7.'}
                {routingSystem === 'SORBNET' && 'Wysokokwotowy (od 1000 PLN), natychmiastowy w godzinach pracy NBP.'}
              </p>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">Bank odbiorcy (BICFI)</label>
              <input type="text" value={receiverBankBicfi}
                onChange={(e) => setReceiverBankBicfi(e.target.value.toUpperCase())}
                placeholder="BANK_B"
                className={`w-full bg-zinc-950 border ${fieldErrors.receiverBankBicfi ? 'border-red-500' : 'border-zinc-800'} rounded-xl px-4 py-3 text-white font-mono text-sm`} />
              {fieldErrors.receiverBankBicfi && <p className="text-red-400 text-xs mt-1.5">{fieldErrors.receiverBankBicfi}</p>}
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">Tytuł</label>
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                maxLength={140} placeholder="np. Faktura 12/2026"
